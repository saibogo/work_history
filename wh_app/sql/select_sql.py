...
def sql_select_point(id_point: str) -> str:
    ...
    Example:
     SELECT point_id, point_name, point_address, CASE WHEN is_work = true THEN 'Open' ELSE 'Closed' END
...
def sql_select_all_works_points() -> str:
    ...
    %(cast_open_close)s FROM %(workspoints)s WHERE %(point_working)s ORDER BY %(point_name)s""" % sql_consts_dict
...
def sql_select_equipment_in_point(point: str) -> str:
    ...
    oborudovanie.point_id WHERE oborudovanie.point_id = 11 ORDER BY oborudovanie.name"""
    ...
    %(oborudovanie)s.%(name)s, %(oborudovanie)s.%(model)s, %(oborudovanie)s.%(serial_num)s, 
    %(oborudovanie)s.%(pre_id)s FROM %(oborudovanie)s JOIN %(workspoints)s ON 
    %(workspoints)s.%(point_id)s = %(oborudovanie)s.%(point_id)s {0} ORDER BY 
    %(oborudovanie)s.%(name)s""" % sql_consts_dict
...
def sql_select_equipment_in_point_limit(point: str, page_num: int) -> str:
    ...
    Example:
        SELECT oborudovanie.id, workspoints.point_name, oborudovanie.name, oborudovanie.model,
        oborudovanie.serial_num, oborudovanie.pre_id FROM oborudovanie JOIN workspoints ON workspoints.point_id =
        oborudovanie.point_id WHERE oborudovanie.point_id = 7 ORDER BY oborudovanie.name LIMIT 5 OFFSET 15
        """

    formatter = ("WHERE %(oborudovanie)s.%(point_id)s = " % sql_consts_dict + str(point))\
        if (str(point) != '' and str(point) != '0') else ''
    query = """SELECT %(oborudovanie)s.%(id)s, %(workspoints)s.%(point_name)s, %(oborudovanie)s.%(name)s, 
    %(oborudovanie)s.%(model)s, %(oborudovanie)s.%(serial_num)s, %(oborudovanie)s.%(pre_id)s 
    FROM %(oborudovanie)s JOIN %(workspoints)s ON %(workspoints)s.%(point_id)s = %(oborudovanie)s.%(point_id)s 
...
def sql_select_work_to_equipment_id(id_equip: str) -> str:
    ...
    Example:
        SELECT * FROM works_likes WHERE id IN (SELECT id FROM works WHERE id_obor = 157) ORDER BY date
        """

    query = """SELECT * FROM %(works_likes)s WHERE %(id)s IN 
    (SELECT %(id)s FROM %(works)s WHERE %(id_obor)s = {0}) ORDER BY %(date)s""" % sql_consts_dict
...
def sql_select_work_from_equip_id_limit(id_equip: str, page_num: int) -> str:
    ...
    Example:
        SELECT * FROM works_likes WHERE id IN (SELECT id FROM works WHERE id_obor = 37) ORDER BY date
    LIMIT 5 OFFSET 25
        """

    query = """SELECT * FROM %(works_likes)s WHERE %(id)s IN 
    (SELECT %(id)s FROM %(works)s WHERE %(id_obor)s = {0}) ORDER BY %(date)s 
...
def sql_select_all_works_limit(page_num: int) -> str:
    ...
    query = """SELECT * FROM %(works_likes)s ORDER BY %(date)s LIMIT {0} OFFSET {1}""" % sql_consts_dict
    return query.format(config.max_records_in_page,
                        (int(page_num) - 1) * config.max_records_in_page)
...
def sql_select_equip_from_like_str(s: str) -> str:
    ...
    Example:
        SELECT id, workspoints.point_name, name, model, serial_num, pre_id FROM oborudovanie
...
def sql_select_point_from_like_str(s: str) -> str:
    ...
    Example:
        SELECT * FROM workspoints WHERE (LOWER(point_name) LIKE LOWER('PoINT')) OR (LOWER(point_address) LIKE
...
def sql_select_point_from_like_str_limit(s: str, page_num: str) -> str:
    ...
    Example:
        SELECT point_id, point_name, point_address,
        CASE WHEN is_work = true THEN 'Работает' ELSE 'Закрыто' END
        FROM workspoints WHERE (LOWER({point_name}) LIKE LOWER('Пб')) OR (LOWER(point_address) LIKE
...
def sql_select_all_works_from_like_str_limit(s: str, page_num: str) -> str:
    """Function return string contain sql-query from table works like all records to s-string
    Example:
        SELECT * FROM works_likes WHERE (LOWER (problem) LIKE LOWER('не Гор')) OR
...
def sql_select_all_works_from_like_str_and_date(s: str, date_start: str, date_stop: str) -> str:
    ...
    Example:
        SELECT * FROM works_likes WHERE ((LOWER (problem) LIKE LOWER('not WORking')) OR (LOWER(result)
...
def sql_select_all_works_from_like_str_and_date_limit(s: str, date_start: str, date_stop: str, page_num: str) -> str:
    """Function return SQL-string contain query from table works like all records to s-string and in dates range
    Example:
        SELECT * FROM works_likes WHERE ((LOWER (problem) LIKE LOWER('not WorK')) OR (LOWER(result)
        LIKE LOWER('not WorK'))) AND (date BETWEEN '21-12-2020' AND '21-12-2019')
...
def sql_select_point_id_from_equip_id(equip_id: str) -> str:
    ...
    return ("""SELECT %(point_id)s FROM %(oborudovanie)s WHERE %(id)s = {0}""" % sql_consts_dict).format(equip_id)


def sql_select_full_equips_info(equip_id: str) -> str:
    """Return SQL-query contain query to complete information from equip
    Example:
        SELECT workspoints.point_name AS point_name, oborudovanie.name AS name, oborudovanie.model AS model,
        oborudovanie.serial_num AS serial_num, oborudovanie.pre_id AS pre_id FROM oborudovanie
        JOIN workspoints ON workspoints.point_id = oborudovanie.point_id WHERE oborudovanie.id = 256
        """

    query = """SELECT %(workspoints)s.%(point_name)s AS %(point_name)s, %(oborudovanie)s.%(name)s AS %(name)s, 
    %(oborudovanie)s.%(model)s AS %(model)s,
     %(oborudovanie)s.%(serial_num)s AS %(serial_num)s, %(oborudovanie)s.%(pre_id)s AS %(pre_id)s 
     FROM %(oborudovanie)s 
     JOIN %(workspoints)s ON %(workspoints)s.%(point_id)s = %(oborudovanie)s.%(point_id)s 
     WHERE %(oborudovanie)s.%(id)s = {0}""" % sql_consts_dict
...
def sql_select_works_from_worker(id_worker: str) -> str:
    ...
    query = """SELECT %(works_from_worker)s.%(id)s, %(point_name)s, %(name)s, %(model)s, 
    %(serial_num)s, %(date)s, %(problem)s, 
     %(result)s, %(all_workers)s FROM %(works_from_worker)s JOIN %(performers)s 
     ON %(performers)s.%(worker_id)s = {0} 
     AND %(works_from_worker)s.%(id)s = %(performers)s.%(work_id)s ORDER BY %(date)s""" % sql_consts_dict
