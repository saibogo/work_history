...
def all_registred_orders_page(page_num: int) -> Response:
    """Return ALL ORDERS page"""
    return goto_or_redirect(lambda: all_registered_orders_table_page(page_num, stylesheet_number()))
...
def all_no_closed_orders_page(page_num: int) -> Response:
    ...
    return goto_or_redirect(lambda: all_no_closed_orders_table_page(page_num, stylesheet_number()))
