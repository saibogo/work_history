...
def sql_select_reading_meter_from_id(id: int) -> str:
    ...
    SELECT reading_lst.%(id)s, %(devices_id)s, %(read_date)s, %(reading)s, %(reading)s - LAG(%(reading)s, 1, 0)
     OVER (ORDER BY %(read_date)s) AS diff, %(Kt)s
    FROM reading_lst
    JOIN %(meter_devices)s ON %(meter_devices)s.%(id)s = {0})
    SELECT %(id)s, %(devices_id)s, %(read_date)s, %(reading)s, diff, %(Kt)s, diff * %(Kt)s AS full_diff FROM tmp""" % sql_consts_dict
