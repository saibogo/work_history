...
def bugs() -> Response:
    ...
    return goto_or_redirect(lambda: bugs_menu(stylesheet_number()), functions.ROLE_CUSTOMER)


@app.route('/all-bugs')
def all_bugs() -> Response:
    """Return all registered problem"""
    return goto_or_redirect(lambda: all_bugs_table(stylesheet_number()), functions.ROLE_CUSTOMER)
...
def all_bugs_paging(page_num: int) -> Response:
    ...
    if is_integer(page_num):
        page = goto_or_redirect(lambda: all_bugs_table_limit(page_num ,stylesheet_number()), functions.ROLE_CUSTOMER)
    else:
...
def all_bugs_in_work() -> Response:
    ...
    return goto_or_redirect(lambda: all_bugs_in_work_table(stylesheet_number()), functions.ROLE_CUSTOMER)
...
def all_bugs_in_work_paging(page_num: int) -> Response:
    ...
    if is_integer(page_num):
        page = goto_or_redirect(lambda: all_bugs_in_work_limit(page_num, stylesheet_number()), functions.ROLE_CUSTOMER)
    else:
        page = flask.abort(code=404)
    return page
...
def invert_bug_status(bug_num: str) -> Response:
    ...
    if is_integer(bug_num):
        page = goto_or_redirect(lambda: create_invert_bug_status_form(bug_num, stylesheet_number()), functions.ROLE_CUSTOMER)
    else:
        page = flask.abort(code=404)
    return page
